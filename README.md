...
- **Style**: **Thumbnails** (default — preview or icon, title, app name) or **Icons** — the native look: large app icons that shrink as the list grows so the whole row fits the screen (96pt down to 48pt, then it scrolls), a filled highlight behind the selection, and the selected item's name floating beneath it at full width, never truncated to the cell. Icons style never captures previews, so it never asks for Screen Recording
