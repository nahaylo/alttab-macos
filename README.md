...
- **Group by Application** is on by default and **Style** defaults to **Icons** — the native look: large app icons that shrink as the list grows so the whole row fits the screen (128pt down to 48pt, then it scrolls) with native proportions for the highlight, spacing and padding, a filled highlight behind the selection, Dock badges (unread counts) on the icons, and the selected item's name floating beneath it at full width, never truncated to the cell. Icons style never captures previews, so it never asks for Screen Recording
